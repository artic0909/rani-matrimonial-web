        ...
        <div class="bg-white/95 backdrop-blur-md rounded-2xl sm:rounded-3xl shadow-2xl border border-white/60 mb-10 overflow-hidden relative z-10">
            ...
            <div class="px-3 sm:px-6 md:px-10 pt-4 sm:pt-8 pb-3 sm:pb-5 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
                <div class="flex items-center gap-2.5 sm:gap-3">
                    <div class="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-rose-700 to-red-800 flex items-center justify-center text-white shadow-xs shrink-0">
                        <svg class="w-4 h-4 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 13l-5 5m0 0l-5-5m5 5V6"/></svg>
                    </div>
                    <div>
                        <div class="flex items-center gap-1.5 sm:gap-2">
                            <h1 class="text-base sm:text-2xl md:text-3xl font-bold font-serif text-gray-900 tracking-wide">Debit History</h1>
                            <span class="px-2 py-0.5 rounded-full text-[9px] sm:text-[11px] font-extrabold bg-rose-100 text-rose-800 border border-rose-200">
                                Spends
                            </span>
                        </div>
                        <p class="text-[10px] sm:text-xs text-gray-500 font-sans mt-0.5 line-clamp-1 sm:line-clamp-none">All deductions for verified contacts & services</p>
                    </div>
                </div>
            ...
            <div class="p-3 sm:p-6 md:p-10 space-y-3 sm:space-y-6 md:space-y-8">
                ...
                <div class="grid grid-cols-3 gap-2 sm:gap-4">
                    ...
                    <div class="p-2.5 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-br from-rose-50 to-white border border-rose-200/80 shadow-2xs flex flex-col justify-between">
                        <div class="flex items-center justify-between">
                            <p class="text-[9px] xs:text-[10px] sm:text-xs font-bold text-rose-800 uppercase tracking-tight sm:tracking-wider truncate">Spent</p>
                    ...
                    <div class="p-2.5 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-br from-slate-50 to-white border border-slate-200 shadow-2xs flex flex-col justify-between">
                        ...
                        <h3 class="text-xs xs:text-sm sm:text-xl md:text-2xl font-extrabold sm:font-bold font-serif text-gray-800 mt-1 truncate">{{ $debitTransactions->total() }} Txns</h3>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-gray-50/80 p-2 sm:p-3 rounded-xl sm:rounded-2xl border border-gray-200/70">
                    <form method="GET" action="{{ route('wallet.debit') }}" class="relative flex-1">
                        <input type="text" 
                               ...
                               placeholder="Search transaction ID, service..." 
                               class="w-full pl-8 sm:pl-9 pr-20 sm:pr-24 py-1.5 sm:py-2 text-xs sm:text-sm rounded-lg sm:rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-rose-500 bg-white shadow-2xs">
                        ...
                        <button type="submit" class="absolute right-1 top-1 sm:right-1.5 sm:top-1 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-rose-700 hover:bg-rose-800 text-white rounded-md sm:rounded-lg text-[11px] sm:text-xs font-bold transition-colors">
                            Search
                        </button>
                    </form>
                    @if(request('search'))
                        <a href="{{ route('wallet.debit') }}" class="px-2.5 py-1.5 sm:px-3 sm:py-2 text-[11px] sm:text-xs font-bold text-gray-500 hover:text-red-600 bg-white border border-gray-200 rounded-lg sm:rounded-xl transition-colors text-center">
                            Clear Filter
                        </a>
                    @endif
                </div>
