    ...
    <div class="mb-8 rounded-2xl bg-gradient-to-r from-emerald-900 via-teal-900 to-emerald-950 p-5 sm:p-6 text-white shadow-xl border border-emerald-500/40 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto relative z-10">
            ...
            <template x-if="!isWpChatAccepted && !isWpChatReceivedByMe && (isWpChatSentByMe || chatRequested || isWpChatCooldown)">
                <button type="button" 
                        disabled
                        class="w-full md:w-auto px-6 py-3 rounded-xl bg-emerald-800/90 text-emerald-200 font-bold text-sm flex items-center justify-center gap-2 shadow opacity-90 cursor-not-allowed">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-[160px]">
            ...
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 p-4">
                ...
                <div class="relative w-full aspect-[4/5] rounded-2xl overflow-hidden bg-gray-900 group cursor-pointer" 
                    ...
                    <div class="absolute top-3 left-3 bg-blue-600 text-white text-[11px] font-extrabold px-3 py-1 rounded-full shadow-lg flex items-center gap-1.5 backdrop-blur-md">
        ...
        <div class="lg:col-span-8 space-y-8">
                ...
                <div class="flex flex-wrap items-start justify-between gap-4 pb-6 border-b border-gray-100">
                    <div>
                        <div class="flex items-center gap-2.5">
                            ...
                            @if($profile->is_bluetick_verified || ($profile->bluetick && (int)$profile->bluetick->is_accept === 1))
                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gradient-to-tr from-blue-600 via-sky-500 to-sky-400 text-white shadow" title="Blue Tick Verified Profile">
                                <svg class="w-3.5 h-3.5 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                            </span>
                            @endif
                        </div>
            ...
            <div class="bg-gradient-to-br from-white via-emerald-50/40 to-teal-50/30 rounded-3xl shadow-xl p-6 sm:p-8 border-2 border-emerald-400/60 relative overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-emerald-200">
                    ...
                    <template x-if="!isWpChatAccepted && !isWpChatReceivedByMe">
                        <button type="button" 
                                ...
                                class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#128C7E] hover:to-[#25D366] disabled:opacity-75 disabled:cursor-not-allowed text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md hover:shadow-lg transition-all cursor-pointer">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div class="p-4 rounded-2xl bg-white border border-emerald-200 shadow-xs flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-base font-extrabold text-gray-900 mt-0.5 block truncate max-w-[200px] sm:max-w-none tracking-wide">{{ $maskedEmail }}</span>
                        </div>
                        <span class="px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-bold">
                            Protected
                        </span>
