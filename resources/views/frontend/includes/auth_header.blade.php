...
<header x-data="{ 
    ...
    <!-- Mobile Navigation Drawer (Off-Canvas Modern Slide-in) -->
    <div x-show="mobileMenuOpen" 
        ...
        <div x-show="mobileMenuOpen" 
             ...
             class="relative flex-1 flex flex-col max-w-xs w-full bg-white shadow-2xl h-full overflow-y-auto">
            
            <!-- User Header Card in Mobile Drawer -->
            <div class="p-5 bg-gradient-to-br from-rani-dark via-rani-primary-dark to-rani-primary text-white relative overflow-hidden">
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="mobileMenuOpen = false" class="p-1 rounded-full text-gray-300 hover:text-white bg-white/10 hover:bg-white/20">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>

                <div class="flex items-center gap-3">
                    <img src="{{ $authCandidate->profile_picture ? asset('storage/' . $authCandidate->profile_picture) : 'https://ui-avatars.com/api/?name='.urlencode($authCandidate->first_name).'&background=D4AF37&color=fff' }}" alt="Profile" class="w-12 h-12 rounded-full border-2 border-rani-gold object-cover shadow-md">
                    <div>
                        <h4 class="font-serif font-bold text-base text-white">{{ $authCandidate->first_name }} {{ $authCandidate->last_name }}</h4>
                        <p class="text-xs text-gray-300 truncate max-w-[170px]">{{ $authCandidate->email }}</p>
                    </div>
                </div>
